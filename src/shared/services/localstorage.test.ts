...
describe('localstorage', () => {
  ...
  describe('localstorage redux middleware', () => {
    ...
    // Helper to create a HistoryState
    ...
    const createAndInvokeMiddlewareWithRetainHistoryFlag = ({
      ...
      edition = 'enterprise',
      version = '4.3.0',
      storageMode = 'localstorage' as 'indexeddb' | 'localstorage'
    }: {
      retain?: boolean
      edition?: string
      version?: string
      ...
      const state = {
        history: createHistoryState(existingHistory, storageMode),
        meta: {
          server: {
            version,
            edition
    ...
    describe('when in localStorage mode', () => {
    ...
    describe('when in IndexedDB mode', () => {
      it('does not sync history to localStorage (IndexedDB handles it)', () => {
        const setItemMock = createAndInvokeMiddlewareWithRetainHistoryFlag({
          retain: true,
          storageMode: 'indexeddb'
        })

        // Should not have called setItem with 'neo4j.history'
        const historyCall = setItemMock.mock.calls.find(
          (call: [string, string]) => call[0] === 'neo4j.history'
        )
