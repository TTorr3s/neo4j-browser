...
Possible values: `auto`, `normal`, `outline`, `dark`, `nord`, `solarizedLight`, `beardedVividBlack`, `beardedOled`, and `beardedMelleJulietLight`.
The theme can be changed in the *Themes* drawer or with `:config`, for example `:config {theme: "nord"}`.
| yes, in the *Themes* drawer
